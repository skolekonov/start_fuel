...
screen -v >/dev/null 2>&1; if [ "$?" == "1" ]; then  echo -n; else { sudo apt-get update >>install.log 2>&1 && sudo apt-get install screen -y >>install.log 2>&1; }; fi || { echo >&2 "'virt-manager' is not available in the path, but it's required. Likely, virt-manager is not installed. Aborting."; exit 1; }
echo_ok
